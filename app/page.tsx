...
export default function Home() {
  ...
  return <main>
    ...
    <SectionCarousel id="community" title="" labels={["Current questions", 'Featured note', 'From the Room']}>
      ...
      <section className="section community-voices carousel-simple"><div className="voices-content room-notes"><div className="room-notes-header"><div><p className="kicker kicker-gold">From the Room</p><h2>Legal AI Notes</h2></div><p>Editorially distilled from a recent discussion; not verbatim quotations. Speakers are unnamed pending approval.</p></div><ol className="room-notes-list">{roomNotes.map((note, index) => <li key={note}><span>0{index + 1}</span><p>{note}</p></li>)}</ol></div></section>
    </SectionCarousel>

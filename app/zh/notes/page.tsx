...
export default function ChineseNotesArchive() {
  return <main lang="zh-CN">
    ...
    <SectionCarousel id="room-notes" title="" labels={roomNotes.map((_, index) => `法律 AI 观点 ${index + 1}`)} locale="zh">
      {roomNotes.map((note, index) => <section className={`note-quote-slide ${index === 2 ? 'is-long' : ''}`} key={note}><div className="note-quote-meta"><p>现场摘记 · 法律 AI 五则</p><span>0{index + 1} / 05</span></div><p className="note-quote-statement">{note}</p><p className="note-quote-disclaimer">根据近期现场讨论整理，并非逐字引述。相关发言人暂不署名，待本人同意后再补充。</p></section>)}
    </SectionCarousel>

...
export default function NotesArchive() {
  return <main>
    ...
    <SectionCarousel id="room-notes" title="" labels={roomNotes.map((_, index) => `Legal AI note ${index + 1}`)}>
      {roomNotes.map((note, index) => <section className={`note-quote-slide ${index === 2 ? 'is-long' : ''}`} key={note}><div className="note-quote-meta"><p>From the Room · Legal AI Notes</p><span>0{index + 1} / 05</span></div><p className="note-quote-statement">{note}</p></section>)}
    </SectionCarousel>
